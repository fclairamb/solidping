...
// Run retires stale platform agents and prunes consumed reconnect nonces, then
// reschedules itself.
//
// Why this job exists (spec 2026-07-27-01): a platform region on fly.io is a
// fleet of machines that each generate their own keypair and enroll on boot —
// that is what lets an app-wide fly secret hold a single multi-use enrollment
// token without any private key ever being shared. The price is that every
// machine replacement leaves an agent row (and its workers row) behind forever.
//
// Org agents are deliberately untouched: they are customer-managed, long-lived,
// and a customer's agent being offline for a fortnight is not a reason for the
...
func (r *AgentGCJobRun) retire(ctx context.Context, jctx *jobdef.JobContext, agent *models.Agent) bool {
	...
	if delErr := jctx.DBService.DeleteWorker(ctx, worker.UID); delErr != nil {
		log.WarnContext(ctx, "Failed to delete retired agent's worker row",
