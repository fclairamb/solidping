...
type Config struct {
	...
	// RunMode is the run mode (test, demo, etc.)
	RunMode string
...
func (s *Service) resetDatabase(ctx context.Context) error {
	slog.InfoContext(ctx, "Resetting PostgreSQL database: dropping all tables")
	...
	slog.InfoContext(ctx, "Database reset complete")
...
func (s *Service) UpdateUser(ctx context.Context, uid string, update *models.UserUpdate) error {
	...
	if update.TOTPRecoveryCodes != nil {
		...
		query = query.Set("totp_recovery_codes = ?::jsonb", string(codesJSON))
	}
	...
	if update.Demo != nil {
		query = query.Set("demo = ?", *update.Demo)
	}
...
// querying — callers page a fixed-size window, so an empty page (e.g. no
// search matches) must not fall through to "no filter at all".
func (s *Service) ListMembersByUsers(
	...
	// Relation("Organization") is a belongs-to, which bun joins inline (a
	// single LEFT JOIN in this same query, not a second round trip). The
	// apply closure's Where runs against the joined "organization" table
	// (bun repoints q.table for the closure's duration) and lands in the main
	// query's WHERE — which, on a LEFT JOIN, turns into a de facto INNER JOIN:
	// a membership whose org is soft-deleted (or missing) has every joined
	// column NULL and gets excluded by "deleted_at IS NULL" rather than kept
	// with Organization left nil.
	err := s.db.NewSelect().
		Model(&members).
...
func (s *Service) UpdateWorkerHeartbeat(
	...
	// Model-driven rather than Set("capabilities = ?", …) on purpose: the raw
	// argument path would JSON-encode the slice on BOTH dialects, which the
	// Postgres `text[]` column rejects. Going through the model lets the bun
	// field tag pick the right per-dialect encoding.
...
// as the reserved word "check". Kept byte-identical to the sqlite twin so the
...
// The retry-on-collision loop lives in db.CreateIncidentWithNumber so both
...
// organization. The unique index on (organization_uid, number) makes this a
// single-row lookup.
...
// whole-column Settings overwrite target the SAME column. Postgres refuses two
// assignments to one column in a single UPDATE, so when both are present the
// flag is folded into the value in Go and written once — which makes the
...
// brandingSettingsMergePG is the two-level JSON merge that writes the
// `branding` section without touching its siblings.
//
// The nesting is load-bearing. A one-level `settings || '{"branding": ...}'`
// would REPLACE the whole branding object — the same last-write-wins clobber
// this merge exists to avoid, just one level down. `||` is shallow in Postgres,
// so the inner concatenation has to be spelled out.
const brandingSettingsMergePG = "settings = settings || jsonb_build_object(" +
	"'branding', coalesce(settings->'branding', '{}'::jsonb) || ?::jsonb)"
...
func (s *Service) UpdateStatusPageBranding(
	...
	patch, err := update.BrandingPatchJSON()
	if err != nil {
		return err
	}
	...
	_, err = s.db.NewUpdate().
		Model((*models.StatusPage)(nil)).
		Set(brandingSettingsMergePG, patch).
		Set("updated_at = ?", time.Now()).
		Where("uid = ?", uid).
		Where("deleted_at IS NULL").
		Exec(ctx)
