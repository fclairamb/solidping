-- v0.17.0 — reap abandoned `created` raw results (spec 2026-08-18-03).
--
...
--
-- Only `created` is reaped, not `running`: models.ResultStatus.IsLifecycleMarker
-- groups both as non-measurement statuses for availability purposes, but
-- `running` is also used by heartbeat checks (handlers/heartbeat) as a
-- legitimate, externally-reported, possibly long-lived status with no
-- check_jobs period/lease for a "plausible execution window" to be measured
-- against — reaping it would finalize a heartbeat's genuine in-progress
-- report into a fake error.
--
