...
// ListStaleCandidates returns the sweep's candidates. The first coalesce
// predicate is the indexed floor (idx_checks_freshness); the second applies
// the exact max(3 × period, 5 min) threshold in SQL, which Postgres can do
// natively on the interval column.
