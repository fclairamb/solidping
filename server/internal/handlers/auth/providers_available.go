...
func (h *ProvidersHandler) ListProviders(writer http.ResponseWriter, _ bunrouter.Request) error {
	...
	if h.cfg.Slack.ClientID != "" && h.cfg.Slack.ClientSecret != "" {
	...
	if h.cfg.Google.ClientID != "" && h.cfg.Google.ClientSecret != "" {
		providers = append(providers, ProviderInfo{
			Name: "Google",
			Type: "google",
		})
	...
	if h.cfg.GitHub.ClientID != "" && h.cfg.GitHub.ClientSecret != "" {
		providers = append(providers, ProviderInfo{
			Name: "GitHub",
			Type: "github",
		})
	...
	if h.cfg.Microsoft.ClientID != "" && h.cfg.Microsoft.ClientSecret != "" {
		providers = append(providers, ProviderInfo{
			Name: "Microsoft",
			Type: "microsoft",
		})
	...
	if h.cfg.GitLab.ClientID != "" && h.cfg.GitLab.ClientSecret != "" {
		providers = append(providers, ProviderInfo{
			Name: "GitLab",
			Type: "gitlab",
		})
	...
	if h.cfg.Discord.ClientID != "" && h.cfg.Discord.ClientSecret != "" {
		providers = append(providers, ProviderInfo{
			Name: "Discord",
			Type: "discord",
		})
