...
// MicrosoftOAuthHandler handles Microsoft OAuth authentication endpoints.
...
func NewMicrosoftOAuthHandler(service *MicrosoftOAuthService, cfg *config.Config) *MicrosoftOAuthHandler {
...
func (h *MicrosoftOAuthHandler) buildMicrosoftAuthURL(state string) string {
	tenant := h.cfg.Microsoft.TenantID
	if tenant == "" {
		tenant = "common"
	}

	params := url.Values{}
	params.Set("client_id", h.cfg.Microsoft.ClientID)
	params.Set("redirect_uri", h.svc.getCallbackURL())
	params.Set("scope", "openid email profile User.Read")
	params.Set("state", state)
	params.Set("response_type", "code")

	return "https://login.microsoftonline.com/" + tenant + "/oauth2/v2.0/authorize?" + params.Encode()
}

// buildSuccessRedirect constructs the redirect URL with tokens.
func (h *MicrosoftOAuthHandler) buildSuccessRedirect(baseURI string, result *MicrosoftOAuthResult) string {
...
func (h *MicrosoftOAuthHandler) handleOAuthError(
	...
	case errors.Is(err, ErrEmailNotVerified):
		...
		description = "No email found in Microsoft profile"
	case errors.Is(err, ErrMicrosoftTokenExchange):
