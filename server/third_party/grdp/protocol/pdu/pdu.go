...
// SendDataPDU is sendDataPDU exported: the SolidPing patch layer
// (patch_solidping.go) sends the TS_SHUTDOWN_REQUEST log-off PDU through the
// same framing every other data PDU takes. Deliberately narrow — one PDU
// writer, not a general escape hatch.
func (p *PDULayer) SendDataPDU(message DataPDUData) {
	p.sendDataPDU(message)
}
	...
	if r.Len() > 0 {
		...
		if p.ShareCtrlHeader.PDUType == PDUTYPE_DEACTIVATEALLPDU {
			...
			if d.Header.PDUType2 == PDUTYPE2_UPDATE {
				...
				if errInfo, ok := d.Data.(*ErrorInfoDataPDU); ok {
					slog.Warn("Received SetErrorInfoPDU from server", "errorInfo", fmt.Sprintf("0x%x (%d)", errInfo.ErrorInfo, errInfo.ErrorInfo))
				}
