...
- **Org-integration targets** (posting to an org's Slack channel or Telegram
  group configured as an org integration) stayed **out of scope**, as the spec
  proposed. Delivery is only ever through each recipient user's own
  notification routes. A follow-up spec can build on the `Notice` type.
- **`support.message` fires for inbound captures only.** An outbound operator
  reply raises nothing: a colleague answering is not something anyone needs to
  be paged for.
- **Recipients fan out over every enabled route.** No per-recipient
  minimum-route picker; a user narrows the fan-out by disabling routes on their
  own notifications page, exactly as they do for incident paging.

### Why no `CHANGELOG.md` edit

Section E asks for "a `CHANGELOG.md` entry". **None was written by hand, on
purpose.** `CHANGELOG.md` is generated by release-please and is only ever
touched by `chore(release):` commits — see
[`wiki/conventions/changelog.md`](../../wiki/conventions/changelog.md): an entry
*is* the `feat:`/`fix:` commit message, expanded into user-facing prose during
the `chore(release)` PR pass. Hand-editing the file would either be overwritten
or would desynchronise release-please's view of the release.

So the entry for this feature is the message of the
`feat(notifications): operators can be paged when a support message arrives or a
new user signs up` commit on this branch. The `notifications` scope is already
in `SCOPE_LABELS`
([`web/docs/src/lib/changelog.ts`](../../web/docs/src/lib/changelog.ts)), so it
renders as "Notifications" on `/docs/changelog` rather than as a raw slug.

### Where the design departs from the spec's sketch

- **`DeliverToUser` takes an `opsnotify.Deps` struct, not a
  `*jobdef.JobContext`.** The sketched signature is unreachable: `jobdef`
  imports `app/services`, whose `Registry` holds a `*support.Service`, and
  `internal/support` is one of the two packages that must be able to raise a
  notice — `support → opsnotify → jobdef → services → support`. The media
  arrive as closures built by `internal/opsnotifywire` instead.
- **The landing organization is resolved by the delivering job, not by the
  raiser.** Every signup path inserts the `users` row *before* the membership,
  so "which org did they land in?" is not answerable at the moment the notice is
  raised. `Notice.AboutUserUID` carries the subject and the job resolves the org
  one queue hop later.
- **The "no notification routes" warning links only on the viewer's own row.**
  The spec asks it to link to "that user's notification settings"; routes are
  per-user and dash0 has no admin route onto another user's, so a link on
  someone else's row could only ever point at the viewer's own page.
