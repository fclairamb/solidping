...
1. **Limits** (`checkicmp/checker.go` `Validate`): count `1–600`, interval `50ms–60s`. Update the affected `checker_test.go` cases (50ms now valid, count ceiling 600).
2. **Budget split** (new `checkerdef.BurstBudgeter` interface + `ICMPConfig.BurstBudget()` + `checkworker/worker.go`): `BurstBudget()` returns the worst-case burst cost `count × timeout + (count-1) × interval` using the same defaults `Execute` applies (timeout 5s, count 1, interval 1s). In `executeJob`, after the config is parsed, a checker config implementing `BurstBudgeter` raises `checkTimeout` to `max(checkTimeout, burstBudget)`; the existing `+1s` margin on `execCtx` stays the grace. The per-packet meaning of `timeout` is untouched: user-set values keep bypassing the cost clamp for the per-packet deadline, but no longer cap the whole burst. Unset `timeout` keeps the `configWithDefaultTimeout` threading (15s per packet), so default count=1 bursts behave exactly as today.
3. **Concurrent burst loop** (`checkicmp/checker.go`): replace the sequential `performICMPPings`/`performSingleICMPPing` with one shared socket (unprivileged UDP, privileged fallback — same as today) and an fping-style split: the calling goroutine writes packet `seq=i` at `i × interval` (ticker), a reader goroutine matches Echo Replies by seq and records `RTT = now − sentAt[seq]` as replies arrive (socket buffer absorbs replies between reads, so RTTs stay honest). Each unanswered packet expires at its own `sentAt + timeout`; total run time is `(count-1) × interval + timeout`, independent of loss. `performICMPPings` returns one result per packet actually written to the socket, plus a fatal error only when the burst could not run at all (socket setup failure, or every write failed).
4. **Truthful accounting** (`checkicmp/checker.go` `Execute`): `packets_sent = len(results)` (only what was really transmitted), loss = `(sent − received) / sent` guarded for `sent == 0` (→ 100, like the DNS-failure path). No partial-result status.
5. **Jitter**: population std-dev over the successful RTTs in the same min/max/avg loop, exposed as `rtt_ms_jitter` (float ms) whenever any reply arrived. No known aggregation suffix → falls through to the type-based default (`float64` → average), so it rolls up like `rtt_ms_avg`.
6. **Tests** (`checkicmp`): limits table updates; `BurstBudget` math; worker budget raising (`checkworker` unit test if the seam allows); burst loop correctness over loopback where ICMP is permitted (CI-skipped like today) plus a pure unit test of the reply-matching/accounting helper; jitter metric presence; truncated-burst accounting (ctx canceled mid-burst → shrunk `packets_sent`, truthful loss).

Out-of-scope note: the check-types metadata (`/api/v1/check-types`) carries per-type period bounds and `secretFields` only — there is no per-config-field bounds mechanism, so the new 50ms/600 bounds are not surfaced there in this spec (spec 2026-09-21-02 owns the form).
