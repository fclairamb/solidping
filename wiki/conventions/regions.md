...
| 9 | Results read filters (`?region=`) | `handlers/results` | org comes from the `/orgs/:org/…` path |
...
`@<org>/<slug>` is still **accepted on input** (check `regions` arrays), so old
API clients, saved exports and bookmarked payloads keep working:
...
on `region`), it would only split it across transactions, and bun runs each
migration file in one transaction anyway. If a future install grows a `results`
table where a one-pass scan at boot is unacceptable, the escape hatch is to
pre-run the same UPDATE manually out of band — the migration is idempotent and
will then find nothing to do.
