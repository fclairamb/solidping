...
- [features/showcase-media.md](features/showcase-media.md) — Regenerable product screenshots and video: the `web/dash0/showcase/` pipeline (`vhs` terminal segment + Playwright dashboard take), `make showcase`, the two-node side-car that puts regions on camera, the cut/time-lapse edit rules, which assets are committed, why the README video is a hand-uploaded GitHub attachment rather than a committed GIF, and the marketing (`solidping-website`) hand-off.
...
- [features/live-demo.md](features/live-demo.md) — The public live demo: the positive-allowlist write guard and the `created_by = NULL` immutability trick, why `PUT …/channels` stays excluded, the "never offer what the guard refuses" frontend rule (and the no-op secondary write that masked a successful edit), the `?demo=true` deep link across all four entry points, and the ownership-bounded dependency-edge extension left on the table.
