...
| auto_created | boolean | Minted by the auto-publish pipeline. Gates the relapse **reopen** path only |
...
| auto_resolve | text | always / if_untouched / never. Applies to every publication **linked** to the resolving incident, hand-published ones included; free-form entries (no `incident_uid`) are never touched |
