...
Catalog of the generated JSON Schemas describing each check type's `config` object, with the path of each one. Auth: public

```json
{
  "data": [{ "checkType": "kubernetes", "ref": "/api/v1/checks/schema/kubernetes" }],
  "note": "These schemas describe a check config for editors and tooling. They are not the validator — ..."
}
```

### GET /api/v1/checks/schema/:type
One check type's config schema, as `application/schema+json` (JSON Schema draft 2020-12). `404` for a type this build does not implement. Auth: public

**These schemas are a description, not a validator.** They are generated by
reflection over the same `XConfig` structs the validators run on
(`server/gen/checkerschema` → `server/internal/checkers/schemas/<type>.json`), so
they mirror the field names and types exactly — but formats, bounds and
cross-field rules stay in the Go `Validate()`, which is the only authority. A
config that satisfies the schema may still be rejected. Validate with
`POST /api/v1/orgs/:org/checks/validate` or `sp checks validate`; never wire a CI
gate onto the JSON instead.

What each document carries beyond plain JSON Schema:

| Key | Meaning |
|---|---|
| `x-solidping-check-type` | The check type the schema describes |
| `x-solidping-validation` | The never-a-validator statement, machine-readable |
| `x-solidping-secret-fields` | Config keys stored encrypted (also `"format": "solidping-secret-ref"` on the property) |
| `x-solidping-notes` | Every rule `Validate()` enforces that the schema does not encode — including each place the struct's `omitempty` tags and the validator disagreed about `required` |
| `x-solidping-go-type` | On a property whose Go type needs explaining (`time.Duration` is a string on the wire) |

`required` is derived from the validator's behaviour, not from `omitempty`: the
generator starts from configs `Validate()` accepts and removes one key at a time.
A key required only in some configurations (prometheus' `metric`, which depends on
`mode`) is therefore absent from `required` and explained in the notes.

The whole set also ships as a release asset,
`check-config-schemas.tar.gz`, for tooling that should not have to reach a
SolidPing instance first.
