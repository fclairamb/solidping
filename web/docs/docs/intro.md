...
```bash
docker run -p 4000:4000 -v solidping-data:/data ghcr.io/fclairamb/solidping:latest
```

The image stores the SQLite database and uploaded blobs (org logos,
status-page assets, screenshots) under `/data`, so a single mounted volume is
enough. See [File Storage](/configuration/file-storage) for details and the
S3 alternative.
