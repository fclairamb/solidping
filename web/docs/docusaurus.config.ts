...
// Label of the API reference category, shared by the sidebar-item filter in the
// docs preset and the explicit category in sidebars.ts. Keep it in sync with
// docs/api/_category_.json.
const API_REFERENCE_LABEL = "API Reference";
    ...
    [
      "classic",
      {
        docs: {
          ...
          // Drop the autogenerated "API Reference" category. The reference is
          // grouped by tag from the generated docs/api/sidebar.ts slice
          // (see sidebars.ts); left in, the autogenerator would ALSO list all
          // 276 operation pages flat, because groupPathsBy: "tag" does not
          // create per-tag subdirectories on disk.
          async sidebarItemsGenerator({ defaultSidebarItemsGenerator, ...args }) {
            const items = await defaultSidebarItemsGenerator(args);

            return items.filter(
              (item) => !(item.type === "category" && item.label === API_REFERENCE_LABEL),
            );
          },
        },
