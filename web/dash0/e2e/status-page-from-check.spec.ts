...
test.describe("Publish a check on a status page", () => {
  test("check detail -> prefilled create form -> the check appears on the page and publicly", async ({
    ...
    try {
      ...
      // Lands on the page detail route with the check already listed under
      // the default "Services" section — no "Add Section" / "Add Component"
      // step.
      ...
      pageUid = statusPageUrl.split("/status-pages/")[1];
      ...
      if (pageUid) {
        await deleteStatusPageViaApi(page, token, pageUid);
      }
      await deleteCheckViaApi(page, token, check.uid as string);
    }
  });
});
