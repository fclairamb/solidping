/**
 * Brand marks for the configurable third-party auth providers, plus the
 * `type` → icon map that resolves them.
 *
 * These lived inline in `routes/orgs/$org/login.tsx` until the register page
 * needed the same grid (spec 2026-09-09-02). They are inline SVG rather than
 * an icon-font/`lucide` lookup because these are brand marks with fixed
 * multi-color paths — `currentColor` would flatten Google's four-color G into
 * a monochrome blob.
 */
...
/**
 * Renders the brand mark for `type`, or nothing when the provider has no
 * registered icon — an instance can expose a provider type this build has
 * never seen, and a missing icon must degrade to a label-only button rather
 * than throw. (This is the `Icon && <Icon …/>` guard the call sites used to
 * repeat, moved inside the component.)
 */
