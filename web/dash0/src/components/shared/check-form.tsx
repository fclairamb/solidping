...
export function CheckForm({
  ...
  function validateSlug(value: string): string | null {
    ...
    if (uuidRegex.test(value)) return t("form.slugMustNotBeUuid");
    if (!slugRegex.test(value)) {
      if (value.length < 3) return t("form.slugMinLength");
      if (value.length > 100) return t("form.slugMaxLength");
      if (!/^[a-z]/.test(value)) return t("form.slugMustStartLowercase");
      return t("form.slugInvalidChars");
    }
    return null;
  }
  ...
  if (slug) orgSummaryParts.push(t("form.summarySlug", { slug }));
  if (labelCount > 0)
    orgSummaryParts.push(t("form.summaryLabelCount", { count: labelCount }));
  if (groupName) orgSummaryParts.push(t("form.summaryGroup", { name: groupName }));
  const orgSummary = orgSummaryParts.join(" · ") || t("form.summarySlugAutoGenerated");
  ...
  const depsSummary =
    depCount > 0
      ? [
          hardDepCount > 0 ? t("form.summaryHardDeps", { count: hardDepCount }) : null,
          softDepCount > 0 ? t("form.summarySoftDeps", { count: softDepCount }) : null,
        ]
          .filter(Boolean)
          .join(", ")
      : t("form.summaryNone");
  ...
  const flappingSummary = t("form.summaryFlapping", {
    window: flappingWindowLabel,
    cooldown:
      reopenCooldownResult.seconds > 0
        ? formatDuration(reopenCooldownResult.seconds)
        : t("form.summaryCooldownOff"),
  }) + (flappingCustomized ? "" : t("form.summaryDefaultsSuffix"));
  ...
  const advancedSummary = [
    timeoutSeconds.trim() !== "" ? t("form.summaryTimeout", { seconds: timeoutSeconds }) : "",
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                    ...
                    <Popover open={typeSearchOpen} onOpenChange={setTypeSearchOpen}>
                      ...
                      <PopoverContent className="p-0 max-h-72">
                        <div className="flex items-center border-b px-3 py-2">
                          ...
                          <input
                            ref={searchInputRef}
                            placeholder={t("form.searchCheckTypes")}
                            value={typeSearch}
                        ...
                        <div className="max-h-56 overflow-y-auto p-1">
                          {filteredCheckTypes.length === 0 ? (
                            <div className="px-3 py-2 text-sm text-muted-foreground">{t("form.noCheckTypesFound")}</div>
                          ) : (
                              ...
                              >
                                ...
                                <div>
                                  <div className="font-medium flex items-center gap-1.5">
                                    {ct.label}
                                    {ct.synthetic && (
                                      <Badge variant="secondary" className="text-[10px] px-1 py-0 font-normal">{t("form.syntheticBadge")}</Badge>
                                    )}
                                  </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="name">{mode === "create" ? t("form.nameOptional") : t("form.name")}</Label>
                <Input id="name" type="text" placeholder={t("form.namePlaceholder")} value={name} onChange={(e) => setName(e.target.value)} data-testid="check-name-input" />
                {mode === "create" && (<p className="text-xs text-muted-foreground">{t("form.nameAutoGeneratedHint")}</p>)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{t("form.scheduling")}</CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                {isPassiveType(type) ? (
                  <div className="flex gap-2">
                    ...
                    <Select value={periodUnit} onValueChange={(v) => setPeriodUnit(v as PeriodUnit)}>
                      ...
                      <SelectContent>
                        {periodUnits.map((u) => (<SelectItem key={u.value} value={u.value}>{t(`form.unit${u.label}`)}</SelectItem>))}
                      </SelectContent>
                    </Select>
                  </div>
                ) : (
                ...
                {type === "heartbeat" && (
                  <p className="text-xs text-muted-foreground">{t("form.heartbeatIntervalHint")}</p>
                )}
                {type === "email" && (
                        ...
                        >
                          ...
                          <SelectContent>
                            {regionSpreadUnits.map((u) => (
                              <SelectItem key={u.value} value={u.value}>{t(`form.unit${u.label}`)}</SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      </div>
          ...
          {authSection && AuthFields && (
            <CollapsibleSection
              ...
              title={t("form.authenticationAndSecrets")}
          ...
          >
              ...
              <div className="space-y-2">
                <Label htmlFor="slug">{mode === "create" ? t("form.slugOptional") : t("form.slug")}</Label>
