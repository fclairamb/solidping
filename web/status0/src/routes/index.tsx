...
// readSpPage reads the server-injected <meta name="sp-page" content="org/slug">
// tag. On a custom domain the server resolves the page from the request host and
// stamps this tag, so the SPA renders that page in place at "/" without
// navigating (the address bar stays on the custom host). Returns null on the
// installation's own hosts, where pages are reached via the URL path instead.
function readSpPage(): { org: string; slug: string } | null {
